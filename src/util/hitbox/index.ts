...
export namespace Hitbox
{
    ...
    export interface Contact<A1 extends Entity.EntityAttributes = Entity.EntityAttributes, A2 extends Entity.Entity<A1> = Entity.Entity<A1>>
    {
        Attacker: A2;
        Attacked: A2;
            ...
            this.hitProcessor = RunService.Heartbeat.Connect(async () =>
            {
                ...
                nonCachedModels.map((m) => [ m, m.FindFirstChildWhichIsA("Humanoid") ] as const)
                    ...
                    .forEach(async ([ hitModel, hitHumanoid ]) =>
                    {
                        ...
                        const entityComponent = Dependency<Components>().getComponent<Entity.Entity>(hitModel);
                        if (entityComponent)
                        {
                            ...
                            const hitboxPossessor = hitboxModel
                            ...
                            const possessorEntityComponent = Dependency<Components>().getComponent<Entity.Entity>(hitboxPossessor);
                            ...
                            if (possessorEntityComponent)
                            {
                                ...
                                for (const listener of ActiveHitbox.onHitListeners)
                                    task.spawn(() => listener.onHit(contactData as never)); // FIXME: this is not correct, remove `never`
                            }
                            else
                            {
